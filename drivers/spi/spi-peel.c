		...
		struct spi_ioc_transfer *u_xfers)
{
	...
	if (u_xfers->rx_buf) {
		if (__copy_to_user((u8 __user *)
			(uintptr_t) u_xfers->rx_buf, photon->buffer, u_xfers->len))
		{
...
static int photon_probe(struct spi_device *spi)
{
	...
	status |= of_property_read_u8(np, "photon,spi-bpw", &spi->bits_per_word);
...
//	photon->ldo_en_gpio = of_get_named_gpio(np, "ldo-en-gpio", 0);
...
MODULE_AUTHOR("xiaotong spreadtrum.com.");
MODULE_LICENSE("GPL");
