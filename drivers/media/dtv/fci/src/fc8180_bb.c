...
s32 fc8180_init(HANDLE handle)
{
	...
	bbm_write(handle, BBM_BUF_SPIOUT, 0x10 | (BBM_I2C_SPI_PHA << 1) |
							(BBM_I2C_SPI_POL));
#endif
#ifdef FEATURE_EXT_LNA
	bbm_write(handle, BBM_FUNCTION_SEL, 0x00);
	bbm_write(handle, BBM_GPIO_DIR, 0x01);
	bbm_write(handle, BBM_GPIO_DATA, 0x01);
#endif
...
s32 fc8180_scan_status(HANDLE handle)
{
	...
	for (; i < ffs_lock_timeout; i++) {
		...
		print_log(handle
			, "[FC8180] ffs_lock_timeout 0x%x, %d\n", data, i);
	...
	print_log(handle
		, "[FC8180] 0x4113(0x%x) 0x4114(0x%x)\n", data, data1);
	...
	for (i = 0; i < ts_err_free_timeout; i++) {
		...
		print_log(handle
			, "[FC8180] ts_err_free_timeout 0x%x, %d\n", data, i);
		if (data & 0x01)
			break;
