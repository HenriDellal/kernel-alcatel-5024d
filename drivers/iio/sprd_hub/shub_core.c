...
static void shub_get_sensor_info(void)
{
	...
	for (i = 0; i < _HW_SENSOR_TOTAL; i++) {
		...
		if (hw_sensor_id[i].id_status == _IDSTA_OK &&
			...
			if((hw_sensor_id[i].chip_id==0x13))
			{
				if(i==0){zyt_info_s2(TB_TYPENAME[i],"DA218"); }
				if(i==3){zyt_info_s2(TB_TYPENAME[i],"STK3210"); }
				if(i==4){zyt_info_s2(TB_TYPENAME[i],"STK3210"); }
			}
			else
			{
			zyt_info_s2(TB_TYPENAME[i],hw_sensor_id[i].pname);
			}
			pr_err("jinq shub_get_sensor_info:%s=%s\n",TB_TYPENAME[i], hw_sensor_id[i].pname);
		}
	}
}
