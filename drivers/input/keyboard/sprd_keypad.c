...
{
	unsigned int mask = BIT_AON_APB_KPD_SOFT_RST;

	regmap_update_bits(sprd_kpd->aon_apb_glb, REG_AON_APB_APB_RST0,
			mask, mask);
	udelay(5); /* add 5us here */
	regmap_update_bits(sprd_kpd->aon_apb_glb, REG_AON_APB_APB_RST0,
			mask, ~mask);
	udelay(5); /* add 5us here */
	regmap_update_bits(sprd_kpd->aon_apb_glb, REG_AON_APB_APB_RTC_EB,
			BIT_AON_APB_KPD_RTC_EB, BIT_AON_APB_KPD_RTC_EB);
	udelay(5); /* add 5us here */
	regmap_update_bits(sprd_kpd->aon_apb_glb, REG_AON_APB_APB_EB0,
			BIT_AON_APB_KPD_EB, BIT_AON_APB_KPD_EB);
}

static void keypad_disable(void)
{
	regmap_update_bits(sprd_kpd->aon_apb_glb, REG_AON_APB_APB_RTC_EB,
			BIT_AON_APB_KPD_RTC_EB, (unsigned int)~BIT_AON_APB_KPD_RTC_EB);
	regmap_update_bits(sprd_kpd->aon_apb_glb, REG_AON_APB_APB_EB0,
			BIT_AON_APB_KPD_EB, (unsigned int)~BIT_AON_APB_KPD_EB);
}

#if DEBUG_KEYPAD
static void dump_keypad_register(void)
{
	pr_info("REG_AON_APB_APB_RST0 = 0x%08x\n",
		aon_kpd_reg_read(REG_AON_APB_APB_RST0));
	pr_info("REG_AON_APB_APB_RTC_EB = 0x%08x\n",
		aon_kpd_reg_read(REG_AON_APB_APB_RTC_EB));
	pr_info("REG_AON_APB_APB_EB0 = 0x%08x\n",
		aon_kpd_reg_read(REG_AON_APB_APB_EB0));
			...
			struct device *dev)
{
	...
	for_each_child_of_node(np, key_np) {
		...
		*keymap++ = KEY(row, col, key_code);
		pr_info("sprd_keypad_parse_dt: %d, %d, %d\n",
			row, col, key_code);
	}
...
static int sprd_keypad_probe(struct platform_device *pdev)
{
	...
	if (!pdata) {
		dev_err(&pdev->dev, "sprd_keypad_probe get platform_data NULL\n");
		return -EINVAL;
	}
