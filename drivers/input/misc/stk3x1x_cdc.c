...
static int32_t stk3x1x_enable_ps(struct stk3x1x_data *ps_data, uint8_t enable)
{
	...
	{		
		printk(KERN_INFO "stk3x1x_enable_ps:first_boot =true\n");
...
#ifdef CONFIG_OF
static struct stk3x1x_platform_data *stk3x1x_accel_parse_dt(struct device *dev)
{
	...
	if (!pdata) {
		dev_err(dev, "Could not allocate struct stk3x1x_platform_data\n");
		return NULL;
	}
	ret = of_property_read_u32(np, "state_reg", &pdata->state_reg);
	if(ret){
		dev_err(dev, "fail to get state_reg\n");
		goto fail;
	}
	ret = of_property_read_u32(np, "psctrl_reg", &pdata->psctrl_reg);
	if(ret){
		dev_err(dev, "fail to get psctrl_reg\n");
		goto fail;
	}
	ret = of_property_read_u32(np, "alsctrl_reg", &pdata->alsctrl_reg);
	if(ret){
		dev_err(dev, "fail to get alsctrl_reg\n");
		goto fail;
	}
	ret = of_property_read_u32(np, "ledctrl_reg", &pdata->ledctrl_reg);
	if(ret){
		dev_err(dev, "fail to get ledctrl_reg\n");
		goto fail;
	}
	ret = of_property_read_u32(np, "wait_reg", &pdata->wait_reg);
	if(ret){
		dev_err(dev, "fail to get wait_reg\n");
		goto fail;
	}
	ret = of_property_read_u32(np, "ps_thd_h", &pdata->ps_thd_h);
	if(ret){
		dev_err(dev, "fail to get ps_thd_h\n");
		goto fail;
	}
	ret = of_property_read_u32(np, "ps_thd_l", &pdata->ps_thd_l);
	if(ret){
		dev_err(dev, "fail to get ps_thd_l\n");
		goto fail;
	}
	ret = of_property_read_u32(np, "transmittance", &pdata->transmittance);
	if(ret){
		dev_err(dev, "fail to get transmittance\n");
		goto fail;
	}
                        ...
                        const struct i2c_device_id *id)
{
	...
	#ifdef CONFIG_OF
	if (np && !plat_data){
		plat_data = stk3x1x_accel_parse_dt(&client->dev);
		if(plat_data){
...
//	printk(KERN_ERR "%s: als_input_dev=0x%x, ps_input_dev=0x%x\n", __func__, ps_data->als_input_dev, ps_data->ps_input_dev);		
	...
	printk(KERN_INFO "%s: probe successfully", __func__);
	{
		...
		zyt_info_s2("[PLSensor] : ","stk3x1x_cdc");
	}
...
#endif	
goto err_als_input_allocate;  //add by jinq for kernel  exceptions stack, I  blieve the interface of input_unregister_device  and  input_free_device  cannt be call together
