...
#define CONFIG_ZYT_PLSENSOR_COMPATIBLE
#define CONFIG_ZYT_SUPPORT

#include <sensors_io.h>

extern int CDC_Plsensor_Device_Id(int);
...
#define ELAN_INT_PIN 216    /*Interrupt pin setting*/
...
#ifdef ZCFG_PSENSOR_YL_DEMAND
#define PS_MIN_MAX_READ		800 // ALex.shi 读到的底噪大于某个值我们就认为是异常的,可能是手遮挡了    
#define PS_MIN_MAX_SET		100 // ALex.shi 读到的底噪大于某个值我们就认为是异常的,可能是手遮挡了    
#endif

#ifdef ZCFG_ENHANCE_INTEGRATE_TIME_FOR_ALS
#define ALS_DYN_INTT	1//0 
#else
#define ALS_DYN_INTT    0
#endif
#define PS_DYN_K        1
#define PS_DYN_K_STR    1
#define HS_ENABLE       0
#define PS_GES          0
...
static int PS_h_offset = 260;
static int PS_l_offset = 800;
...
static const char ElanPS_Name[] = "epl2590_pls";
#else
#if S5PV210
...
static void initial_global_variable(struct i2c_client *client, struct epl_sensor_priv *obj)
{
...
#endif
	epl_sensor.als.gain = EPL_GAIN_LOW; //EPL_GAIN_MID;
...
#ifdef ZCFG_ENHANCE_INTEGRATE_TIME_FOR_ALS
	c_gain = 400; // 300/1000=0.3 0921
#else
    c_gain = 300; // 300/1000=0.3 0921
#endif
	...
	#if defined(ZCFG_ENHANCE_INTEGRATE_TIME_AND_THRESHOLD) || defined(ZCFG_PSENSOR_YL_DEMAND)
	...
	#if defined(ZCFG_ENHANCE_INTEGRATE_TIME_AND_THRESHOLD) || defined(ZCFG_PSENSOR_YL_DEMAND)
    dynk_low_offset = 150;         //low thd offset
    dynk_high_offset = 250;        //high thd offset
	#else
    dynk_low_offset = PS_LOW_THRESHOLD;         //low thd offset
    dynk_high_offset = PS_HIGH_THRESHOLD;        //high thd offset
	#endif
    dynk_change_thd_max = 30000;   //change thd condition
    dynk_thd_offset = 0; //500;         //change thd offset
#if PS_DYN_K_STR
    dynk_enhance_max_ch0 = 50000; //enhance max ch0
#ifdef ZCFG_ENHANCE_INTEGRATE_TIME_FOR_ALS
    dynk_enhance_integration_time = EPL_ALS_INTT_1024;
#else
	...
	#ifdef ZCFG_ENHANCE_INTEGRATE_TIME_AND_THRESHOLD
...
#if defined(ZCFG_PSENSOR_YL_DEMAND)
		if (dynk_min_ps_raw_data > PS_MIN_MAX_READ)
			dynk_min_ps_raw_data = PS_MIN_MAX_SET;
#endif
...
static void epl_sensor_polling_work(struct work_struct *work)
{
...
#endif
	LOG_INFO("enable_als = %d, als.polling_mode = %d \n", enable_als, epl_sensor.als.polling_mode);
	...
	LOG_INFO("enable_ps = %d, ps.polling_mode = %d \n", enable_ps, epl_sensor.ps.polling_mode);
...
#endif
    err = request_irq(epld->irq,epl_sensor_eint_func, IRQF_TRIGGER_FALLING, //IRQF_TRIGGER_FALLING, //IRQF_TRIGGER_LOW
...
static struct device_attribute cdc_als_enable_attribute = __ATTR(als_enable,0664,light_enable_show,light_enable_store);
//static struct device_attribute cdc_als_poll_delay_attribute = __ATTR(als_delay,0666,cdc_als_delay_show,cdc_als_delay_store);

static struct device_attribute cdc_ps_enable_attribute = __ATTR(ps_enable,0664,proximity_enable_show,proximity_enable_store);
...
static int epl_sensor_probe(struct i2c_client *client,const struct i2c_device_id *id)
{
...
#ifndef CONFIG_ZYT_SUPPORT	// Alex.shi AndroidL报重启,delete  
    ...
    LOG_INFO("sensor probe success.\n");
	{
		...
		zyt_info_s2("[PLSensor] : ","epl259x_cdc");
	}
