...
#include ZCFG_TP_FOCALTECH_AUTO_UPGRADE
};
#else
static unsigned char CTPM_FW[] ={};
#endif
...
int fts_ctpm_fw_upgrade_with_i_file(struct i2c_client *client)
{
	...
	*/
   printk("fts_updateinfo_curr.CHIP_ID:0x%x\n",fts_updateinfo_curr.CHIP_ID);
...
//	printk(&client->dev, "bootloader version = 0x%x\n", reg_val[0]);
...
int fts_ctpm_auto_upgrade(struct i2c_client *client)
{
	...
	printk("[FTS] read chip id is 0x%x\n",uc_reg_value);
...
/*the chip id of FT6306 is 0x06*/
	...
	uc_host_fm_ver = fts_ctpm_get_i_file_ver();
       ...
       printk("[FTS]chenqx uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",uc_tp_fm_ver, uc_host_fm_ver);
	...
	if (/*the firmware in touch panel maybe corrupted */
		...
		msleep(100);
		printk("[FTS] uc_tp_fm_ver < uc_host_fm_ver,uc_tp_fm_ver = 0x%x, uc_host_fm_ver = 0x%x\n",
				uc_tp_fm_ver, uc_host_fm_ver);
	...
	printk("[FTS]********************Exit CTP Auto Upgrade********************\n");
	return 0;
}
...
//	int i_ret;
	...
	for (i = 0; i < FTS_UPGRADE_LOOP; i++) {
		...
		printk("(spl)[FTS] Step 1:Reset  CTPM\n");
