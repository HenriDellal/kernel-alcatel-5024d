			 ...
			 unsigned int cmd)
{
	...
	if (unlikely(atomic_read(&dev->stream_on) == 0)) {
		pr_warn("fail to idx:%d stream not on, skip captrue req.\n",
				dev->idx);
		ret = -EAGAIN;
		goto exit_nolock;
	}
	...
	if (!is_dual_cam) {
		...
		if (ret) {
			...
			sprd_isp_wait_for_buffers((void *)isp_dev_first);
			...
			sprd_isp_wait_for_buffers((void *)isp_dev_second);
	...
	if (is_dual_cam) {
		...
		if (dev_other->cap_flag == DCAM_CAPTURE_STOP) {
			if (isp_dev->delta_full)
				...
				pr_err("fail to resume...\n");
		} else {
			pr_info("postpone path resume...(%d)\n", dev->idx);
			/* let dcam 1 start streaming, so ISP can do its job */
			if (!isp_dev->delta_full)
				dcam_path_resume(dev_other->idx);
			else
				pr_err("fail to resume...\n");
