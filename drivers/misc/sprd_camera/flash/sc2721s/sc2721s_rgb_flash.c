...
#if defined(ZCFG_SC2721S_FLASH_RGB_CURRENT)
	regmap_update_bits(sprd_rgb_flash_handle,
				ANA_REG_GLB_RGB_CTRL,
				BITS_RGB_V(ZCFG_SC2721S_FLASH_RGB_CURRENT), BITS_RGB_V(ZCFG_SC2721S_FLASH_RGB_CURRENT));
#else
	regmap_update_bits(sprd_rgb_flash_handle,
				ANA_REG_GLB_RGB_CTRL,
				...
				struct flash_driver_data *drv_data)
{
	...
	pr_info("sprd_sc2721s_leds_bltc_rgb_enable\n");
}
...
static int sprd_flash_rgb_probe(struct platform_device *pdev)
{
	...
	printk("sprd_flash_rgb_probe  start\n");
	...
	ret = of_property_read_u32(node, "rgb-reg", &pmic_base_addr);
	if (ret) {
		pr_info("rgb-reg read dts error!\n");
		goto exit;
	} else {
