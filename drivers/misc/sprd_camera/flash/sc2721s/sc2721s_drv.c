					  ...
					  struct sprd_flash_element *element)
{
	...
	if (!drv_data)
		return -EFAULT;
#if defined(ZCFG_SC2721S_FLASH_STEADY_CURRENT)
	 element->index = ZCFG_SC2721S_FLASH_STEADY_CURRENT; /* 0-15 */
#elif defined(ZCFG_SC2721S_FLASH_MAX_CURRENT)	
	if(element->index >ZCFG_SC2721S_FLASH_MAX_CURRENT)
	  element->index =ZCFG_SC2721S_FLASH_MAX_CURRENT;
	...
	if (SPRD_FLASH_LED1 & idx)
		regmap_update_bits(drv_data->reg_map,
			FLASH_CTRL_REG, FLASH_V_SW, element->index);
	pr_info("%s: idx:%d  element->index: %d", __FUNCTION__, idx, element->index);
