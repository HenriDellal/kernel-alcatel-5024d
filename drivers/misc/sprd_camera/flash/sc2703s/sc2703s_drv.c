...
void sc2703_charge_set_buck_mode(void *drvd)
{
	struct flash_driver_data *drv_data = (struct flash_driver_data *)drvd;

	if (drv_data->charger_reg_map) {
		regmap_write(drv_data->charger_reg_map, 0xc0, 0xa1);
		regmap_write(drv_data->charger_reg_map, 0x85, 0x40);
		regmap_write(drv_data->charger_reg_map, 0xfc, 0x00);
		regmap_write(drv_data->charger_reg_map, 0xc0, 0x00);
	}
}
...
/*
	regmap_update_bits(drv_data->reg_map,
		SC2703_FLASH_DRIVER_ACTIVE,
		1 << (SC2703_LED1_EN_SHIFT + idx - 1),
		(enable == 0 ? 0 : 1)
		<< (SC2703_LED1_EN_SHIFT + idx - 1));

	regmap_update_bits(drv_data->reg_map,
		SC2703_FLASH_DRIVER_ACTIVE,
		1 << (SC2703_LED1_DRIVER_EN_SHIFT + idx - 1),
		(enable == 0 ? 0 : 1)
		<< (SC2703_LED1_DRIVER_EN_SHIFT + idx - 1));
*/
	return 0;
}
...
static int sprd_flash_sc2703s_open_torch(void *drvd, uint8_t idx)
{
	...
	if (!drv_data)
		return -EFAULT;
//	printk("sprd_flash_sc2703s_open_torch\n");

	idx = drv_data->torch_led_index;
	sc2703_charge_set_buck_mode(drvd);
...
static int sprd_flash_sc2703s_close_torch(void *drvd, uint8_t idx)
{
	...
	if (!drv_data)
		return -EFAULT;
//	printk("sprd_flash_sc2703s_close_torch\n");
...
static int sprd_flash_sc2703s_open_preflash(void *drvd, uint8_t idx)
{
	...
	sc2703_flash_set_mode(drvd, LED_TORCH);
	sc2703_flash_led_enable(drvd, idx, 1);
//	printk("sprd_flash_sc2703s_open_preflash\n");
...
static int sprd_flash_sc2703s_close_preflash(void *drvd, uint8_t idx)
{
	...
	if (!drv_data)
		return -EFAULT;
//	printk("sprd_flash_sc2703s_close_preflash\n");
...
static int sprd_flash_sc2703s_open_highlight(void *drvd, uint8_t idx)
{
	...
	if (!drv_data)
		return -EFAULT;
//	printk("sprd_flash_sc2703s_open_highlight\n");
...
static int sprd_flash_sc2703s_close_highlight(void *drvd, uint8_t idx)
{
	...
	if (!drv_data)
		return -EFAULT;
//	printk("sprd_flash_sc2703s_close_highlight\n");
...
//	printk("sprd_flash_sc2703s_cfg_value_preflash-element->index:%d\n",element->index);
					   ...
					   struct sprd_flash_element *element)
{
	...
	if (!drv_data)
		return -EFAULT;
#if defined(ZCFG_SC2703S_FLASHLIGHT_MAX_CURRENT)
		if(element->index > ZCFG_SC2703S_FLASHLIGHT_MAX_CURRENT){
			element->index=ZCFG_SC2703S_FLASHLIGHT_MAX_CURRENT;
	} 

		
//		printk("sprd_flash_sc2703s_cfg_value_highlight- ZCFG_SC2703S_FLASHLIGHT_MAX_CURRENT :%d\n",ZCFG_SC2703S_FLASHLIGHT_MAX_CURRENT);
#endif
	regmap_write(drv_data->reg_map,
			SC2703_FLASH_FD_CONFIG6 + idx/2 * 2, element->index);
//	printk("sprd_flash_sc2703s_cfg_value_highlight-element->index:%d\n",element->index);
					   ...
					   struct sprd_flash_element *element)
{
	...
	if (!drv_data)
		return -EFAULT;
    #if defined(ZCFG_SC2703S_TOUCH_MAX_CURRENT)
		if(element->index > ZCFG_SC2703S_TOUCH_MAX_CURRENT){

			element->index=ZCFG_SC2703S_TOUCH_MAX_CURRENT;
	} 
		idx = drv_data->torch_led_index;
		regmap_write(drv_data->reg_map,
			SC2703_FLASH_FD_CONFIG5 + idx/2 * 2, element->index);
	...
	regmap_write(drv_data->reg_map,
			SC2703_FLASH_FD_CONFIG5 + idx/2 * 2, element->index);
    #endif
	printk("sprd_flash_sc2703s_cfg_value_torch-element->index:%d\n",element->index);
	return 0;
}
		...
		const struct i2c_device_id *id)
{
	...
	np = of_parse_phandle(drv_data->dev->of_node, "sc2703,charger", 0);
	if (IS_ERR_OR_NULL(np))
		pr_err("failed to get sc2703,charger node\n");
	else {
		pdev = of_find_device_by_node(np);
		if (IS_ERR_OR_NULL(pdev)) {
			pr_err("failed to get sc2703,charger pdev\n");
			of_node_put(np);
		} else {
			drv_data->charger_reg_map =
					dev_get_regmap(pdev->dev.parent, NULL);
			if (IS_ERR(drv_data->charger_reg_map)) {
				pr_err("failed to regmap for sc2703,charger\n");
				drv_data->charger_reg_map = NULL;
			}
		}
	}
...
void sprd_flash_sc2703s_shutdown(struct i2c_client *client)
{
	int idx = 1;////or 0 please test it
	sc2703_flash_led_enable(sc2703_drv_data, idx, 0);
	printk("sprd_flash_sc2703s_shutdown---end\n");
//eturn 0;
}
