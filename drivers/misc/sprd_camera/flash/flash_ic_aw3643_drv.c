...
#define FLASH_TEST
#define FLASH_IC_DRIVER_NAME "sprd_aw3643"
/* Slave address should be shifted to the right 1bit.
 * R/W bit should NOT be included.
 */
#define I2C_SLAVEADDR	0x63

#define AW3643_LEVEL_NUM 12
...
#if 0
/*IFLASH(mA)¡Ö(Brightness Code*2.95mA)+2.55mA */
...
static const unsigned char aw3643_flash_level[AW3643_LEVEL_NUM] = {
	12, 12, 46, 50, 55, 59, 63, 67, 72, 76, 80, 84};
...
static int sprd_flash_ic_init(void *drvd)
{
	...
	if (drv_data->i2c_info) {
		...
		ret = flash_ic_driver_reg_write(drv_data->i2c_info, 0x04, 0x0f);
		ret = flash_ic_driver_reg_write(drv_data->i2c_info, 0x06, 0x0f);
		ret = flash_ic_driver_reg_write(drv_data->i2c_info, 0x06, 0x1f);
...
static int sprd_flash_ic_open_torch(void *drvd, uint8_t idx)
{
	...
	if (drv_data->i2c_info)
		ret = flash_ic_driver_reg_write(drv_data->i2c_info, 0x01, 0x0a);
...
static int sprd_flash_ic_open_highlight(void *drvd, uint8_t idx)
{
	...
	if (drv_data->i2c_info)
		ret = flash_ic_driver_reg_write(drv_data->i2c_info, 0x01, 0x0e);
					  ...
					  struct sprd_flash_element *element)
{
	...
	if (drv_data->i2c_info) {
			flash_ic_driver_reg_write(drv_data->i2c_info, 0x06,
				element->index*4);
			drv_data->flash_cfg.torch_level = element->index*4;
		}
					  ...
					  struct sprd_flash_element *element)
{
	...
	if (drv_data->i2c_info) {
		if (SPRD_FLASH_LED0 & idx) {
			flash_ic_driver_reg_write(drv_data->i2c_info, 0x05, 0);
			flash_ic_driver_reg_write(drv_data->i2c_info, 0x06,
						element->index*4);
			drv_data->flash_cfg.preflash_level = element->index*4;
			}
		}
					   ...
					   struct sprd_flash_element *element)
{
	...
	/*i = aw3643_verify_level(element->index*4);*/
	drv_data->flash_cfg.highlight_level = element->index*4;
	level = element->index*4;
	if (drv_data->i2c_info) {
		if (SPRD_FLASH_LED0 & idx) {
			pr_err("%s %d %d %d\n", __func__, __LINE__, idx, level);
			flash_ic_driver_reg_write(drv_data->i2c_info, 0x03, 0);
