...
#define CHARGE_HIGHORLOW_RESTART
...
extern int s_cur_vbat,s_cur_ocv,s_cur_current,s_cur_temp;
extern __s64 s_use_time;
...
static int sprdbat_is_chg_timeout(void)
{
	...
	use_time = cur_time.tv_sec - sprdbat_data->bat_info.chg_start_time;
	s_use_time = use_time;
	if (use_time >
...
static void sprdbat_current_adjust_by_status(uint32_t status)
{
	...
	if (status == 0) {
		...
		if (sprdbat_data->bat_info.chg_stop_flags &
		...
		#ifdef CHARGE_HIGHORLOW_RESTART
			if (sprdbat_data->bat_info.cur_temp >
				sprdbat_data->pdata->otp_low_restart){
				SPRDBAT_DEBUG("[low temp] restart charge,cur_temp = %d,otp_low_restart = %d\n"
							 ,sprdbat_data->bat_info.cur_temp,sprdbat_data->pdata->otp_low_restart);
				sprdbat_change_module_state(SPRDBAT_OTP_COLD_RESTART_E);
				}
		#else
			SPRDBAT_DEBUG("bat is  already cold restart\n");
			sprdbat_change_module_state(SPRDBAT_OTP_COLD_RESTART_E);
		#endif
			...
			sprdbat_data->pdata->otp_high_restart){
				SPRDBAT_DEBUG("[high temp] restart charge,cur_temp = %d,otp_high_restart = %d\n"
							 ,sprdbat_data->bat_info.cur_temp,sprdbat_data->pdata->otp_high_restart);
				sprdbat_change_module_state(SPRDBAT_OTP_OVERHEAT_RESTART_E);
			}
	    #else
				SPRDBAT_DEBUG("bat is  already hight restart\n");
				sprdbat_change_module_state
...
static void sprdbat_print_battery_log(void)
{
	...
	/*-------for zytinfo start-----------*/
		s_cur_vbat = sprdbat_data->bat_info.vbat_vol;
		s_cur_ocv = sprdbat_data->bat_info.vbat_ocv;
		s_cur_current = sprdbat_data->bat_info.bat_current;
		s_cur_temp = sprdbat_data->bat_info.cur_temp;
	/*-------for zytinfo end------------*/
...
int __weak sprdbat_get_bat_id_through_IDpin(struct platform_device *pdev)
{
	int temp ,ret = 0;
	struct device_node *np = pdev->dev.of_node;
	temp = of_get_named_gpio(np, "battery-id-gpios", 0);
	if (gpio_is_valid(temp)) 
	{
		if(!gpio_request(temp,"batttery_id_gpio"))
		{
			gpio_direction_output(temp, 1);
			udelay(1000);
			gpio_direction_input(temp);
			if(gpio_get_value(temp)==0)
			{
				ret = 0;
				SPRDBAT_DEBUG("%s:get batttery_id_gpio value is: %d\n",__func__,gpio_get_value(temp));//gpio==0.使用一供电池
			}
			else
			{
		 		ret = 1;
				SPRDBAT_DEBUG("%s:get batttery_id_gpio value is: %d\n",__func__,gpio_get_value(temp));//gpio==1.使用二供电池
			}

		}
		else
		{
			SPRDBAT_DEBUG("%s:request batttery_id_gpio is fail !\n",__func__);
			return 0;
		}
	} 
	else 
	{
		SPRDBAT_DEBUG("%s:battery-id-gpio is invalid\n",__func__);
		return 0;
	}
	return ret;
...
static int (*sprdbat_bat_fun[4])(struct platform_device *pdev) = {
	sprdbat_get_bat_id,
    sprdbat_get_bat_id_through_IDpin,
};
...
#ifdef CHARGE_HIGHORLOW_RESTART
				i = pdata->jeita_tab_size - 1;
				pdata->otp_high_restart = pdata->jeita_tab[i].w;  //get the last data !
				pdata->otp_low_restart  = pdata->jeita_tab[0].w;  //get the first data !
				pdata->otp_high_stop  = pdata->jeita_tab[i].x;
				pdata->otp_low_stop  = pdata->jeita_tab[0].x;	
#endif
