...
void sc2703_charger_input_current_set(unsigned int val)
{
	...
	ret = regmap_read(sc2703_chg->regmap, SC2703_GPIO_CONF_A, &gpio_conf_value);
	if (ret)
		return;
	...
	if(iin_lim0 == 3) {
		...
		if (gpio_conf_value == 0x02) {
			regmap_update_bits(sc2703_chg->regmap,
					   SC2703_REG_UNLOCK,
					   SC2703_CHG_REG_MASK_ALL,
					   SC2703_REG_UNLOCK_VAL);
			ret = regmap_read(sc2703_chg->regmap,
					  SC2703_INPUT_LIMIT_CURRENT,
					  &limit_input_value);
			if (ret)
				return;

			limit_input_value += 0x48;
			regmap_update_bits(sc2703_chg->regmap,
					   SC2703_INPUT_LIMIT_CURRENT,
					   SC2703_CHG_REG_MASK_ALL,
					   limit_input_value);
...
void sc2703_charger_otg_enable(int enable)
{
	if (enable) {
		...
		regmap_update_bits(sc2703_chg->regmap, SC2703_REG_UNLOCK,
			   SC2703_CHG_REG_MASK_ALL, SC2703_CHG_REG_DISABLE_VAL);
		regmap_update_bits(sc2703_chg->regmap, SC2703_DCDC_CTRL_F,
				   SC2703_DCDC_REV_VOUT_MASK,
				   SC2703_CHG_DCDC_REV_VOUT_5000MV);
		sc2703_dcdc_output_set_cur(SC2703_IIN_REV_LIM_900MA);
		regmap_update_bits(sc2703_chg->regmap, SC2703_DCDC_CTRL_A,
...
static void sc2703_otg_restart(void)
{
	...
	sc2703_dcdc_output_set_cur(SC2703_IIN_REV_LIM_1200MA);
	regmap_update_bits(sc2703_chg->regmap, SC2703_DCDC_CTRL_A,
...
static int sc2703_charger_probe(struct platform_device *pdev)
{
	...
	ret = regmap_update_bits(sc2703_chg->regmap, SC2703_DCDC_CTRL_F,
				 SC2703_DCDC_REV_VOUT_MASK, 0);
	if (ret)
		return ret;
...
#ifdef ZCFG_SC2703_15S_POWEROFF
	/* Enable the detection of long key presses */
	ret = regmap_update_bits(sc2703_chg->regmap, SC2703_CONF_A,
