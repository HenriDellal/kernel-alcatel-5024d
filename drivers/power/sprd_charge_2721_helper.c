...
int s_cur_cnom=0,s_cur_rint=0,s_bat_id=0,s_cur_track_status = 4,s_cur_vbat=0,s_cur_ocv=0,s_cur_current=0,s_cur_temp=0;
__s64 s_use_time=0;
char * zyt_battery_info(void)
{
	static char info[500]={0};
	char temps[100]={0};

	//	info = (char*)kmalloc(sizeof(sizeof(char)*500), GFP_KERNEL);
//	if (info == NULL) {
//		err = -ENOMEM;
//		printk("zyt_battery_info alloc memory fail\n");
//	}
	
	if(pbat_data ==NULL) return "";
	
	sprintf(info,"------------------------------------------\n[BAT_ID] %d\n[BAT] %d-%d-%d-%d \n[VBAT] %d [OCV] %d [CURRENT] %d\n[Cnom] %d [Rint] %d\n[Time] %lld\n",
		s_bat_id,
		pbat_data->rechg_vol,
		pbat_data->chg_end_vol_l,
		pbat_data->chg_end_vol_pure,
		pbat_data->chg_end_cur,
		s_cur_vbat,
		s_cur_ocv,
		s_cur_current,
		s_cur_cnom,
	...
	if(pbat_data->cnom_track_support == 1)
	{
		if(s_cur_track_status == 0)
		{
			strcat(info,"[Track] QMAX_INIT\n");
		}
		else if(s_cur_track_status == 1)
		{
			strcat(info,"[Track] QMAX_IDLE\n");
		}
		else if(s_cur_track_status == 2)
		{
			strcat(info,"[Track] QMAX_UPDATING\n");
		}
		else if(s_cur_track_status == 3)
		{
	...
	if(pbat_data->temp_support == 1)
	{
		sprintf(temps,"[Temp] %d\n[OTP] %d-%d-%d-%d\n",
			s_cur_temp,
			pbat_data->otp_low_stop,
			pbat_data->otp_low_restart,
			pbat_data->otp_high_restart,
			pbat_data->otp_high_stop
			);
		strcat(info,temps);
	}
	else
	{
		strcat(info,"[Temp] Not Support");
	}
	return info;
