...
void fan54015_set_vindpm(unsigned char reg_val)
{
	...
	if (vendor_id == VENDOR_PSC54X5A) {// For PSC5415A
		reg_val |= VSP4P533;	// page 29 in datasheet,VSP bit 2 must be 1

		//Set ADD20MV 1 to increase 20mV in OREG value( page 29 in datasheet)
		fan54015_set_value(FAN5405_REG_SP_CHARGER, (0x01<<7),
				   7, (0x01<<7));
	}
...
void fan54015_init(void)
{
	...
	if (vendor_id == VENDOR_PSC54X5A) {
		...
		//reg 1
		fan54015_set_value(FAN5405_REG_CONTROL1, FAN5405_IINLIM, FAN5405_IINLIM_SHIFT,NOLIMIT);  // INLIM = 500mA bit7,bit6
		//fan54015_set_value(FAN5405_REG_CONTROL1, FAN5405_TE, FAN5405_TE_SHIFT, ENTE);  // INLIM = 500mA bit5 stop
		//reg 2
		//fan54015_set_value(FAN5405_REG_OREG, FAN5405_OREG,FAN5405_OREG_SHIFT, VOREG4P37);  //OREG = 4.35V
		//reg 4
		//fan54015_set_value(FAN5405_REG_IBAT, FAN5405_IOCHARGE, FAN5405_IOCHARGE_SHIFT, IOCHARGE1450);  //max Icharge
		//reg 5
		fan54015_set_value(FAN5405_REG_SP_CHARGER, FAN5405_VSP, FAN5405_VSP_SHIFT, VSP4P533);  //IO_LEVEL is 0. Output current is controlled by IOCHARGE bits.
		for (i = 0; i < 11; i++) {
			fan54015_read_reg(i, &data);
			SPRD_EX_DEBUG("fsc5425_ReadReg1 i = %d, data = %x\n",
				i, data);
		}
	} else {
		fan54015_sw_reset();
...
{
	return fan54015_get_value(FAN5405_REG_IC_INFO,(0x7 << 0),0);
}
