...
static int sc2703_bl_set_pwm(struct sc2703_bl *bl, int brightness)
{
	 ...
	 */
	pwm_duty = bl->pwm->period * brightness / bl->max_brightness;

	ret = pwm_config(bl->pwm, pwm_duty, bl->pwm->period);
	if (ret) {
...
static int sc2703_update_brightness(struct sc2703_bl *bl, int brightness)
{
	...
	#ifdef ZCFG_SC2703_BACKLIGHTNESS_LEVEL
	brightness =( brightness * ZCFG_SC2703_BACKLIGHTNESS_LEVEL)/100;
	#endif
...
static int sc2703_bl_wled_init(struct sc2703_bl *bl)
{
	...
	if (set_pwm_in_range) {
		unsigned int period2Freq = 1000000 / bl->pwm->period;
...
static int sc2703_backlight_probe(struct platform_device *pdev)
{
	...
	if (ret)
		return ret;
#if defined(ZCFG_SC2703_BACKLIGHT_DEFAULT_VALUE)
	return sc2703_update_brightness(bl, ZCFG_SC2703_BACKLIGHT_DEFAULT_VALUE);
#else
