...
/*add by alex.shi for zyt_info*/
static char zyt_peripherals_LCD_name[26] = {0};
...
static struct device_node *panel_device_node_get(struct device_node *np,
					 int panel_id)
{
		int i;
		int count;
	...
	for (index = 0; (index < 2) && panel_id[index]; index++) {
		...
		else
		{
			...
			if	( udc_lcd_create(SEC_LCD0, pd) ){
			  sprintf(zyt_peripherals_LCD_name, "%s  [CLK]:%d", panel->lcd_name, panel->phy_freq/1000);
			}
			
		
			mipi_timing_from_dt(&panel->display_timing, panel);
		}	
		pd->panel->of_node=panel_device_node_get(pdev->dev.of_node,0xffff);////add jinq for Sub Pmic 2703 regulator enable and disable in suspend & resume
#else
