...
static void dpu_y2r_coef_cfg(struct dispc_context *ctx,
			struct sprd_adf_hwlayer *hwlayer)
{
	int reg_val = 0;
	struct dpu_reg *reg = (struct dpu_reg *)ctx->base;

	if (hwlayer->y2r_coef == prev_y2r_coef)
		return;

	switch (hwlayer->y2r_coef) {
	case SPRD_COLORSPACE_BT601_CONFORMANT_RANGE:
		reg_val = BIT(4);
		break;
	case SPRD_COLORSPACE_BT601_FULL_RANGE:
		reg_val = 0;
		break;
	case SPRD_COLORSPACE_BT709_CONFORMANT_RANGE:
		reg_val = BIT(4) | BIT(5);
		break;
	case SPRD_COLORSPACE_BT709_FULL_RANGE:
		reg_val = BIT(5);
		break;
	case SPRD_COLORSPACE_BT2020_CONFORMANT_RANGE:
		reg_val = BIT(6) | BIT(4);
		break;
	case SPRD_COLORSPACE_BT2020_FULL_RANGE:
		reg_val = BIT(6);
		break;
	default:
		break;
	...
	prev_y2r_coef = hwlayer->y2r_coef;

	dpu_stop(ctx);
	reg->dpu_cfg0 &= ~(0x7 << 4);
	reg->dpu_cfg0 |= reg_val;
	dpu_run(ctx);
}
...
static void dpu_enhance_set(struct dispc_context *ctx, u32 id, void *param)
{
	...
	struct cm_cfg *cm;
	#if !defined(ZCFG_DISABLE_CFG_ID_SLP)
	struct slp_cfg *slp;
	#endif
